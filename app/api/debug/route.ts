...
import { debugErrorStream } from '@/lib/ai/gemini';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { errorMessage, code, context } = body;

    // Validación
    if (!errorMessage || typeof errorMessage !== 'string' || errorMessage.trim().length === 0) {
      return new Response(
        JSON.stringify({ error: 'El mensaje de error es requerido' }),
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          console.error('[API Debug] Error en stream:', error);
          const errorMessage = error instanceof Error ? error.message : 'Error desconocido';
          console.error('[API Debug] Stack trace:', error instanceof Error ? error.stack : 'N/A');
